					...
					inout PointStream<PARTICLE_INPUT> particleStream)
{
	...
	else if(gIn[0].Type == ACTIVE_EMITTER)
	{
		if(gIn[0].Age > 0.1f)
		{
			PARTICLE_INPUT p;
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 10.0f;
			...
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 20.0f;
			p.Size		= float2(10.0f, 10.0f);
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
			...
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 30.0f;
			p.Size		= float2(10.0f, 10.0f);
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
			...
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 40.0f;
			p.Size		= float2(10.0f, 10.0f);
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
...
{
	return SampleParticleTexture(PARTICLE_GUN_SPARKS_ID, (half2)input.TexC);// * input.Color;
}
