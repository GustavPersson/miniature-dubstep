...
void* ServerGroup::socketFunc(void *args)
{
	...
	int nRet;
	do{
			nRet = recvfrom(group->mListeningSocket, (char *)&port, sizeof(int), 0,
							(struct sockaddr*)&addrSrc, &addrSrcSize);
	...
	if(group->mConnectionManager != NULL)
	{
		...
		nRet = connect(connection, (LPSOCKADDR)&serverInfo, sizeof(struct sockaddr));
		if(nRet==NETWORK_OK)
