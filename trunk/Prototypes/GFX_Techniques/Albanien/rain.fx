	...
	{	
		...
		if( gIn[0].age > 0.0001f )
		{
			for(int i = 0; i < 5; ++i)
			{
				...
				float3 vRandom = 512.0f*RandVec3((float)i/5.0f);
				vRandom.y = 20.0f;
			
				Particle p;
				p.initialPosW = gEmitPosW.xyz + vRandom;
				p.initialVelW = float3(0.0f, -40.0f, 0.0f);
...
VS_OUT DrawVS(Particle vIn)
{
	...
	float temp = sin(time) * 0.101f;
	float3 acc = {temp, -9.8f, 0.0f};
	vOut.posW = 0.5f*t*t*acc + t*vIn.initialVelW + vIn.initialPosW;
