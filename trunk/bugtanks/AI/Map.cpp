		...
		for (int x = 0; x < pPotentialUnit->getPotentialSize().x; x++)
		{
			...
			if (validIndex(currentIndex))
			{
				...
				if (mpPotentialField[arrayOffset] != INVALID_POTENTIAL_VALUE)
				{
					...
					UPDATE_MAP_POTENTIAL_ON_SPOT(arrayOffset);
				}
			}
		}
	}
...
{
	removePotentialUnit(keyId, false);
}

void Map::removePotentialUnit(const logic::KeyType& keyId, bool keepUnit)
{
	...
	if (it != mPotentialUnits.end())
	{
		...
		// Get the position of the unit when we added it to the map's potential field
		utilities::MapCoordinate mapPosOffset = pPotentialUnit->getPenultimateMapPosition();

		mapPosOffset -= pPotentialUnit->getPotentialSize() >> 1;

		// Get the index offset in the map's potential field
		const utilities::MapCoordinate& indexOffset = indexInFieldByPosition(mapPosOffset);

		// This is the actual position of the building, used when fetching it's value from
		// getPotentialValue
		utilities::MapCoordinate unitPosOffset = pPotentialUnit->getMapPosition();

		unitPosOffset -= pPotentialUnit->getPotentialSize() >> 1;

		// Remove the unit from the map's potential field
		for (int y = 0; y < pPotentialUnit->getPotentialSize().y; y++)
		{
			int arrayOffsetY = (indexOffset.y + y) * mMapSize.x;
			utilities::MapCoordinate unitPos(0, unitPosOffset.y + y);
			utilities::MapCoordinate mapPos(0, mapPosOffset.y + y);

			for (int x = 0; x < pPotentialUnit->getPotentialSize().x; x++)
			{
				unitPos.x = unitPosOffset.x + x;
				mapPos.x = mapPosOffset.x + x;
				int arrayOffset = arrayOffsetY + indexOffset.x + x;

				float value = pPotentialUnit->getPotentialValue(unitPos);
				...
				else if (value == INVALID_POTENTIAL_VALUE)
				{
					...
					recalculatePositions.add(mapPos);
				}
			}
		}
		...
		{
			recalculateValueOnPositions(recalculatePositions, keyId);
		}
	}
}
...
void Map::updatePositionForUnit(const logic::KeyType& keyId)
{
	// Check if we have the unit
	std::map<logic::KeyType, const PotentialUnit*>::iterator it = mPotentialUnits.find(keyId);
	if (it != mPotentialUnits.end())
	{
		// Remove the potential unit and then re-add it	
		removePotentialUnit(keyId, true);
		addPotentialUnit(it->second);
	}
}

void Map::recalculateValueOnPositions(utilities::VectorList<utilities::MapCoordinate>& positions, const logic::KeyType& skipUnitId)
{
	while (!positions.empty())
	{
		...
		// Iterate through all the potential units, skip the rest of the iterations if the map's position is invalid
		std::map<logic::KeyType, const PotentialUnit*>::iterator it = mPotentialUnits.begin();
		while(it != mPotentialUnits.end() && mpPotentialField[arrayOffset] != INVALID_POTENTIAL_VALUE)
		{
			// Check for skipping unit
			if (it->second->getKey() == skipUnitId)
			{
				++it;
				continue;
			}
...
#ifdef AI_DRAW_MAP_POTENTIAL_FIELD
void Map::updateMapPotentialOnSpot(int arrayOffset)
{
