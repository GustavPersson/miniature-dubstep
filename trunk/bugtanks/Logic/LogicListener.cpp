...
bool LogicListener::handleEvent(const logic::EventData &mEvent)
{
	...
	else if (EvtData_System_EnterEditorMode::msEventType == mEvent.getEventType())
	{
		if (Application::getInstance()->mpGameOptions->mIsServer)
		{
			if (Application::getInstance()->mpGameOptions->mNrOfEditors == 0)
			{
				safeThreadSafeQueueEvent(logic::EventDataPtr(myNew ai::EvtData_To_AiTogglePause()));
			}
			Application::getInstance()->mpGameOptions->mNrOfEditors++;
		}
	}
	else if (EvtData_System_ExitEditorMode::msEventType == mEvent.getEventType())
	{
		if (Application::getInstance()->mpGameOptions->mIsServer)
		{
			Application::getInstance()->mpGameOptions->mNrOfEditors--;
