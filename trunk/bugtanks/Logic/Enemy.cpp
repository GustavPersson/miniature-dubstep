...
void Enemy::reduceHp(float damage, int networkId, KeyType shooter)
{
	
	mHealthPoints -= damage;
	safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_From_EnemyHitNetwork(shooter)));
	...
	if(mHealthPoints <= 0 && !mDead && mId.isOwner())
	{
		...
		if(mEnemyType == Enemy::EnemyType_KamikazeAnt)
		{
			mpsDXModule->getParticles("BigExplosion")->addEmitter(mPosition.convertToD3DXVECTOR3());
		}
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_From_ActorDestroyed(this->getKey(), networkId)));
		DEBUG_MESSAGE(LEVEL_LOWEST, "enemyreducehp");
		mDead = true;
	}
}
