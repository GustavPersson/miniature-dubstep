...
FireSpit::FireSpit(float damage, float weaponCooldown, float range, const KeyType& ownerId):
...
void FireSpit::fireWeapon(const KeyType& targetId)
{
    ...
    if(pOwner != NULL && pTarget != NULL && mCooldownCounter > EnemyWeapon::getCooldown())
    {
		...
		if(mBullets <= 1)
		{
			safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorCreateMunition(Munitions::MunitionType_Fire, position, aimDir, mOwner)));
		}
