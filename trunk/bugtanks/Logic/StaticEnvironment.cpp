...
StaticEnvironment::StaticEnvironment(int networkId, StaticEnvironmentTypes type, utilities::Position pos, utilities::Direction dir, utilities::Vec3Float scale) : StaticActor(pos, dir, networkId)
{
	...
	gfx::InstanceMesh* pMesh = dynamic_cast<gfx::InstanceMesh*>(mpTransformObject);
	if (pMesh != NULL)
	{
		gfx::InstanceSubset* pSubSet = pMesh->getSubsetByIndex(0);
		if (pSubSet != NULL)
		{
			gfx::OBB* pObb = dynamic_cast<gfx::OBB*>(pSubSet->getBoundingVolume());
			if (pObb != NULL)
			{
				pObb->scale(scale.convertToD3DXVECTOR3());
				mSize.x = (int)(pObb->getSizeX() * 2.0f * utilities::WORLD_IN_MAP_COORDS) + 1;
				mSize.y = (int)(pObb->getSizeZ() * 2.0f * utilities::WORLD_IN_MAP_COORDS) + 1;
			}
		}
	}
