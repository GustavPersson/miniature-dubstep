...
void Minigun::fireWeapon(KeyType ownerId)
{
	...
	if(mCooldownCounter > Weapon::getCooldown())
	{
		...
		if(mBulletKey != INVALID_KEY)
		{
			...
			mpDX10Module->getParticles("GunSparks")->addEmitter(origin,dir);
			mpLogicQuadTree->collisionRay(origin, dir, PlayerInfo::getInstance()->getPlayerId(), pActor->getKey(), false);
			safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_From_WeaponFireRayMunition(position, direction,PlayerInfo::getInstance()->getPlayerId(), pActor->getKey(), false)));
		}		
	}
}
