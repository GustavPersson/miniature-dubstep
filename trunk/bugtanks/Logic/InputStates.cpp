...
void InputStates::menuState()
{
	...
	if(letterKeysPressed != 0 || keysPressed != 0)
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_System_KeysPressed(letterKeysPressed, keysPressed)));
}

void InputStates::gameInsideTankState()
{
	//Show the ingame HUD.
	HUDHandler::getInstance()->show();
	...
	{
		letterKeysPressed |= LetterKey_Q;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStartRotate(mpPlayerInfo->getPlayerId(), true, true)));
	}
	if(wasKeyPressed(DIK_W))
	{
		letterKeysPressed |= LetterKey_W;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStartMove(mpPlayerInfo->getPlayerId(), true)));
	}
	if(wasKeyPressed(DIK_E))
	{
		letterKeysPressed |= LetterKey_E;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStartRotate(mpPlayerInfo->getPlayerId(), false, true)));
	}
	if(wasKeyPressed(DIK_R))
	{
		letterKeysPressed |= LetterKey_R;
	...
	{
#ifdef _DEBUG
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_From_DriverEnterTank(mpPlayerInfo->getPlayerId())));
#endif
		letterKeysPressed |= LetterKey_O;
	}
	if(wasKeyPressed(DIK_P))
	...
	{
		letterKeysPressed |= LetterKey_A;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStartRotate(mpPlayerInfo->getPlayerId(), true, false)));
	}
	if(wasKeyPressed(DIK_S))
	{
		letterKeysPressed |= LetterKey_S;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStartMove(mpPlayerInfo->getPlayerId(), false)));
	}
	if(wasKeyPressed(DIK_D))
	{
		letterKeysPressed |= LetterKey_D;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStartRotate(mpPlayerInfo->getPlayerId(), false, false)));
	}
	if(wasKeyPressed(DIK_F))
	{
		letterKeysPressed |= LetterKey_F;
	...
	{
		letterKeysPressed |= LetterKey_N;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_CameraStartRotate(true)));
	}
	if(wasKeyPressed(DIK_M))
	{
		letterKeysPressed |= LetterKey_M;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_CameraStartRotate(false)));
	}
	if(wasKeyPressed(DIK_SPACE))
	{
		keysPressed |= Key_Space;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankStartFire(mpPlayerInfo->getPlayerId())));
	}
	if(wasKeyPressed(DIK_ESCAPE))
	{
		Application::getInstance()->resetGame();
	}
	if(wasKeyPressed(DIK_RETURN))
	{
		keysPressed |= Key_Return;
		...
		if(Application::getInstance()->mpGameOptions->mEditorMode || Application::getInstance()->mpGameOptions->mEnableEditor)
		{
			...
			safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_System_EnterEditorMode()));
		}
#endif
	}
	if(wasKeyPressed(DIK_DOWN))
	...
	{
		keysPressed |= Key_Ctrl;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankStartFireSecondary(mpPlayerInfo->getPlayerId())));
	}
	if(wasKeyPressed(DIK_RCONTROL))
	{
		keysPressed |= Key_Ctrl;
	...
	if(wasKeyPressed(DIK_1))
	{
		if(ActorHandler::getInstance()->getActor(mpPlayerInfo->getPlayerId())->getActorSubType() == Tank::TankType_CloseCombat)
		{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankChangeWeapon(mpPlayerInfo->getPlayerId(), Weapon::WeaponTypes_Minigun)));
		}
		else
		{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankChangeWeapon(mpPlayerInfo->getPlayerId(), Weapon::WeaponTypes_Cannon)));
		}
		keysPressed |= Key_1;
	}
	if(wasKeyPressed(DIK_2))
	{
		if(ActorHandler::getInstance()->getActor(mpPlayerInfo->getPlayerId())->getActorSubType() == Tank::TankType_CloseCombat)
		{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankChangeWeapon(mpPlayerInfo->getPlayerId(), Weapon::WeaponTypes_Flamethrower)));
		}
		else
		{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankChangeWeapon(mpPlayerInfo->getPlayerId(), Weapon::WeaponTypes_GaussCannon)));
		}
		keysPressed |= Key_2;
	}
	if(wasKeyPressed(DIK_3))
	{
		if(ActorHandler::getInstance()->getActor(mpPlayerInfo->getPlayerId())->getActorSubType() == Tank::TankType_CloseCombat)
		{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankChangeWeapon(mpPlayerInfo->getPlayerId(), Weapon::WeaponTypes_PulseCannon)));
		}
		else
		{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankChangeWeapon(mpPlayerInfo->getPlayerId(), Weapon::WeaponTypes_BeamCannon)));
		}
		keysPressed |= Key_3;
	}
	if(wasKeyPressed(DIK_4))
	...
	if(wasKeyReleased(DIK_Q))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStopRotate(mpPlayerInfo->getPlayerId(), true, true)));
	}
	if(wasKeyReleased(DIK_W))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStopMove(mpPlayerInfo->getPlayerId(), true)));
	}
	if(wasKeyReleased(DIK_E))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStopRotate(mpPlayerInfo->getPlayerId(), false, true)));
	}
	if(wasKeyReleased(DIK_R))
	{
	}
	...
	if(wasKeyReleased(DIK_A))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStopRotate(mpPlayerInfo->getPlayerId(), true, false)));
	}
	if(wasKeyReleased(DIK_S))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStopMove(mpPlayerInfo->getPlayerId(), false)));
	}
	if(wasKeyReleased(DIK_D))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStopRotate(mpPlayerInfo->getPlayerId(), false, false)));
	}
	if(wasKeyReleased(DIK_F))
	{
	}
	...
	if(wasKeyReleased(DIK_N))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_CameraStopRotate(true)));
	}
	if(wasKeyReleased(DIK_M))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_CameraStopRotate(false)));
	}
	if(wasKeyReleased(DIK_SPACE))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankStopFire(mpPlayerInfo->getPlayerId())));
	}
	if(wasKeyReleased(DIK_RETURN))
	{
	}
	...
	if(wasKeyReleased(DIK_LCONTROL))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_TankStopFireSecondary(mpPlayerInfo->getPlayerId())));
	}
	if(wasKeyReleased(DIK_RCONTROL))
	{
	}
...
void InputStates::gameOutsideTankState()
{
	...
	{
		letterKeysPressed |= LetterKey_Q;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_DriverStrafe(mpPlayerInfo->getPlayerId(), true,true)));
	}
	if(wasKeyPressed(DIK_W))
	{
		letterKeysPressed |= LetterKey_W;
	...
	{
		letterKeysPressed |= LetterKey_E;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_DriverStrafe(mpPlayerInfo->getPlayerId(), false,true)));
	}
	if(wasKeyPressed(DIK_R))
	{
		letterKeysPressed |= LetterKey_R;
	...
	{
		keysPressed |= Key_Space;
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_DriverStartFire(mpPlayerInfo->getPlayerId())));
	}
	if(wasKeyPressed(DIK_ESCAPE))
	{
	...
	if(wasKeyReleased(DIK_Q))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_DriverStrafe(mpPlayerInfo->getPlayerId(), true,false)));
	}
	if(wasKeyReleased(DIK_W))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorStopMove(mpPlayerInfo->getPlayerId(), true)));
	...
	if(wasKeyReleased(DIK_E))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_DriverStrafe(mpPlayerInfo->getPlayerId(), false,false)));
	}
	if(wasKeyReleased(DIK_R))
	{
	}
	...
	if(wasKeyReleased(DIK_SPACE))
	{
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_DriverStopFire(mpPlayerInfo->getPlayerId())));
	}
	if(wasKeyReleased(DIK_RETURN))
	{
	}
...
void InputStates::editorState()
{
	...
	if(wasKeyPressed(DIK_RETURN))
	{
		...
		if(Application::getInstance()->mpGameOptions->mEditorMode || Application::getInstance()->mpGameOptions->mEnableEditor)
		{
			...
			safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_System_ExitEditorMode()));
		}
#endif
	}
	if(wasKeyPressed(DIK_DOWN))
...
void InputStates::allStates()
{
	...
	{
#ifdef _DEBUG
		safeThreadSafeQueueEvent(EventDataPtr(myNew ai::EvtData_To_AiTogglePause()));
#endif
	}
	...
	// God Mode :D
	if (wasKeyPressed(DIK_F10))
	{
#ifdef _DEBUG
		safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_PlayerToggleGodMode()));
#endif
	}
	else if (wasKeyReleased(DIK_F10))
	{
