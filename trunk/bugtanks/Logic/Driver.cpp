...
void Driver::reduceHp(float damage)
{
	if (!mGodMode)
	{
		...
		if(mHealthPoints <= 0 && !mDead && mId.isOwner())
		{
			...
			safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_ActorCreateSpectator(mPosition, mDirection, info)));
			safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_To_InputChangeGameState(logic::InputStates::GameState_GameOver)));
			safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_From_ActorDestroyed(this->getKey(), -3)));
			DEBUG_MESSAGE(LEVEL_LOWEST, "driverreducehp");
			mDead = true;
		}
	}
}
...
void Driver::enterTank()
{
  ...
  //if a tank is in range the driver will jump in
	...
	safeThreadSafeQueueEvent(EventDataPtr(myNew EvtData_From_ActorDestroyed(this->getKey(), -3)));
	DEBUG_MESSAGE(LEVEL_LOWEST, "driverentertank");
