...
ParticleGunSparks::ParticleGunSparks(ID3D10Device *pDevice, Effect *pEffect, char* name, int passIndex, int layoutIndex, int initParticles, int maxChildParticles, int offset) 
:Particle(pDevice, pEffect, name, passIndex, layoutIndex, initParticles, maxChildParticles, offset)
{
	for(int i = 0; i < initParticles; ++i)
	{
		mpEmitterData[i].size = D3DXVECTOR2(1.0f, 1.0f);
	}
}
