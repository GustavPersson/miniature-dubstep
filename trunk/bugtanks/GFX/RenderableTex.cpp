...
HRESULT RenderableTex::createBackBuffer()
{
	...
	if(FAILED(hr))
	{
		ERROR_MESSAGE("Failed to retrieve back buffer from swap chain.")
		return hr;
	}
	...
	hr = mpDevice->CreateRenderTargetView(backBuffer, &RTVDesc, &mpRTV);
	if(FAILED(hr))
	{
		ERROR_MESSAGE("Failed to create backbuffer render target view.")
		return hr;
	}
...
HRESULT RenderableTex::createRenderTarget()
{
	...
	hr = mpDevice->CreateRenderTargetView(temp, &RTVDesc, &mpRTV);
	if(FAILED(hr))
	{
		ERROR_MESSAGE("Failed to create render target view.")
		return hr;
	}
	...
	if(FAILED(hr))
	{
		ERROR_MESSAGE("Failed to create render shader resource view.")
		return hr;
	}
...
HRESULT RenderableTex::createDepthStencil()
{
	...
	if(FAILED(hr))
	{
		ERROR_MESSAGE("Failed to create depth stencil view.")
		return hr;
	}
	...
	if(FAILED(hr))
	{
		ERROR_MESSAGE("Failed to create depth shader resource view.")
		return hr;
	}
...
HRESULT RenderableTex::createDepthStencilMS()
{
	HRESULT hr;
	D3D10_TEXTURE2D_DESC texDesc;
	ZeroMemory(&texDesc, sizeof(D3D10_TEXTURE2D_DESC));
	...
	D3D10_DEPTH_STENCIL_VIEW_DESC depthDesc;
	depthDesc.ViewDimension = D3D10_DSV_DIMENSION_TEXTURE2DMS;
	depthDesc.Format = DXGI_FORMAT_D32_FLOAT;
