...
bool EditorFileHandler::saveMap(const char *mapName, bool overwrite)
{
	...
	//Get the TriggerHandler instance.
	TriggerHandler *pTriggerHandler = TriggerHandler::getInstance();

	MapDataTrigger triggerData;
	vector<TriggerBase *> triggerVector;
	pTriggerHandler->getAllTriggers(triggerVector);

	//Write the number of triggers.
	unsigned int nTriggers = triggerVector.size();
	write((const char *)&nTriggers, sizeof(unsigned int), location);

	//Write all triggers.
	for(vector<TriggerBase *>::iterator triggerIter = triggerVector.begin();
		triggerIter != triggerVector.end(); ++triggerIter)
	{
		if(EditorMapHandler::packTrigger((*triggerIter), triggerData))
		{
			write((const char *)&triggerData, sizeof(MapDataTrigger), location);
		}
	}
...
bool EditorFileHandler::loadMap(const char *mapName)
{
	...
	//Send network plane update events.
	EditorMapHandler::sendMapWeights(planeData, planeWeights);
	...
	//Read the number of triggers.
	read((char *)&nTriggers, sizeof(unsigned int), location);

	//Read all triggers and send creation events.
	MapDataTrigger triggerData;
	for(n = 0; n < nTriggers; ++n)
	{
		read((char *)&triggerData, sizeof(MapDataTrigger), location);
		EditorMapHandler::createTriggerFromData(triggerData);
	}
