...
void EditorGUI::init()
{
	...
	mpTriggerWindow = myNew EditorGUITriggerWindow(getInstanceBase(), getClientWidth(), getClientHeight(), 0);
	mpTriggerWindow->setRelativePosition(D3DXVECTOR2(0.22f, 0.02f));
	mpTriggerWindow->setRelativeSize(D3DXVECTOR2(0.56f, 0.96f));
	mpTriggerWindow->setColor(D3DXCOLOR(0.5f, 0.5f, 0.5f, 0.9f));
	addFrame(mpTriggerWindow);

	mpUtilityBar->setTriggerWindow(mpTriggerWindow);
}