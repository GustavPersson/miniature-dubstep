...
void* Connector::socketFunc(void *args)
{
	...
	{
		ERROR_MESSAGE("ERROR(Connector::socketFunc): pthread_setcancelstate -> PTHREAD_CANCEL_ENABLE");
	}
	if(pthread_setcanceltype (PTHREAD_CANCEL_ASYNCHRONOUS, NULL))
	{
		ERROR_MESSAGE("ERROR(Connector::socketFunc): setcanceltype -> PTHREAD_CANCEL_ASYNCHRONOUS" << endl);
	}
...
bool Connector::startAccept(u_short port)
{
	int nret;
...
bool Connector::connectTo(const char *addr, u_short port, SOCKET *connection)
{
	 int nret; // ErrorCode
