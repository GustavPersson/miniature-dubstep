...
void ConnectionManager::removeAllMessageHandlers()
{
	pthread_mutex_lock(&mMutex);
	for(list<network::MessageHandler *>::iterator handlerIter = mMessageHandlers.begin();
		handlerIter != mMessageHandlers.end(); ++handlerIter)
	{
		if((*handlerIter))
		{
			delete (*handlerIter);
