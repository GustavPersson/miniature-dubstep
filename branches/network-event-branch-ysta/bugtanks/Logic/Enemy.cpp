...
Enemy::Enemy(double hp,double damage,Sizes size, int aiStyle,int range)
{
	mpEnemyWeapon = NULL;
	mHealthPoints = hp;
	mDamage = damage;
...
void Enemy::calculateDamage(/*event*/)
{
  //attacking monsters damage is subtracted from targeted players healthpool
  //send event to tank to reduce healthpool by damage
}
