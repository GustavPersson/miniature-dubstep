...
public:
	...
	**/
	explicit BaseEventData(const float timeStamp = GlobalTimer::getInstance()->getTime(Timer::ReturnType_MilliSeconds))
...
public:
	...
	*/
	explicit EmptyEventData(const float timeStamp = GlobalTimer::getInstance()->getTime(Timer::ReturnType_MilliSeconds)) 
