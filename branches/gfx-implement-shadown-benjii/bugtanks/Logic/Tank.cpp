...
Tank::Tank(double hp,int armor,TankTypes tankType)
{
	...
	{
		mpRenderObject = gfx::DX10Module::getInstance()->createMeshInstanced("CloseRangeTank");
		mpRenderObject->setTextureID(1);
		dynamic_cast<gfx::InstanceMesh*>(mpRenderObject)->setFlagUpdateAbsolute(false);
		mpRenderObject->setID(mId);
		mpRenderObject->setName("CloseRangeTank");
	...
	else
	{
		...
		mpRenderObject->setTextureID(0);
		dynamic_cast<gfx::InstanceMesh*>(mpRenderObject)->setFlagUpdateAbsolute(false);
		mpRenderObject->setID(mId);
		mpRenderObject->setName("LongRangeTank");
		mpRenderObject->setPosition(D3DXVECTOR3(mPosition.x, mPosition.y, mPosition.z));
		dynamic_cast<gfx::InstanceMesh*>(mpRenderObject)->setScale(D3DXVECTOR3(2.5f, 1.5f, 2.5f));
...
void Tank::update(float deltaTime)
{
	...
	if(mRotating)
	{
		...
		float angle = atan2(1.0f, 0.0f) - atan2(mDirection.z, mDirection.x);
		dynamic_cast<gfx::TransformObject*>(mpRenderObject)->setRotationY(angle);
		angle = atan2(1.0f, 0.0f) - atan2(mSubsetDirection.z, mSubsetDirection.x);
		dynamic_cast<gfx::InstanceMesh*>(mpRenderObject)->getSubsetByIndex(1)->setRotationY(angle);
	}
	if(mSubsetRotating)
	{
		mSubsetDirection.rotateAroundY(TANK_ROTATE_SPEED * deltaTime * mSubsetRotateDirection);
		mSubsetDirection.normalize();
		float angle = atan2(1.0f, 0.0f) - atan2(mSubsetDirection.z, mSubsetDirection.x);
		dynamic_cast<gfx::InstanceMesh*>(mpRenderObject)->getSubsetByIndex(1)->setRotationY(angle);
	}
	gfx::DX10Module::getInstance()->getCamera()->setPosition(mPosition.x - 100.0f, 200.0f, mPosition.z - 100.0f);
	gfx::DX10Module::getInstance()->getCamera()->setTarget(mPosition.x, mPosition.y, mPosition.z);
}
