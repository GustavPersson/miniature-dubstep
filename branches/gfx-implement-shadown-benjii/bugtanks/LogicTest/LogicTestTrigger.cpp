...
void testTrigger1()
{
	...
	safeAddListener(handler->getEventListenerPointer(), EvtData_From_ActorDestroyed::mEventType);
	safeAddListener(handler->getEventListenerPointer(), EvtData_System_StartGame::mEventType);
	DEBUG_MESSAGE("EventListener: " << handler->getName())

	//Send event to fullfill the requirements of Parent.
	safeQueueEvent(EventDataPtr(myNew EvtData_From_ActorDestroyed(5, 0.0f)));
	manager->tick(10);

	Sleep(3000);

	//Send event to fullfill the requirements of FirstChild.
	safeQueueEvent(EventDataPtr(myNew EvtData_From_ActorDestroyed(1, 0.0f)));
	manager->tick(10);

	Sleep(3000);

	//Send event to fullfill the requirements of ChildOfFirst.
	safeQueueEvent(EventDataPtr(myNew EvtData_From_ActorDestroyed(0, 0.0f)));
	manager->tick(10);

	Sleep(3000);

	//Send event to fullfill the requirements of SecondChild.
	safeQueueEvent(EventDataPtr(myNew EvtData_From_ActorDestroyed(4, 0.0f)));
	manager->tick(10);

	Sleep(3000);

	DEBUG_MESSAGE("Sending empty event...")
