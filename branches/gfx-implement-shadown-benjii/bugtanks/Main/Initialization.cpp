...
void checkHardDisk(const DWORDLONG diskSpaceNeeded)
{
	...
	if (mDiskFree.avail_clusters < neededClusters)
	{
		...
		ERROR_MESSAGE(90001, "You don't have enough disk space to run the game!");
...
void checkMemory(const DWORDLONG physicalRAMNeeded, const DWORDLONG virtualRAMNeeded)
{
	...
	if (mMemStatus.ullTotalPhys < physicalRAMNeeded)
	{
		...
		ERROR_MESSAGE(90002, "You don't have enough physical memory, loser.");
	...
	if (mMemStatus.ullAvailVirtual < virtualRAMNeeded)
	{
		...
		ERROR_MESSAGE(90003, "You don't have enough virtual memory. Try closing other programs.");
		throw ErrorHandler(90003);
	}

	char *mBuffer = myNew char[virtualRAMNeeded];
	...
	{	
		...
		ERROR_MESSAGE(90004, "You don't have enough contiguous RAM. Try closing some other programs.");
...
GameOptions::GameOptions(const char *path)
{
	...
	mMusicVolume = ::GetPrivateProfileIntA(
		"SOUND", "Music_Volume", false, path) / 100.0f ;

	mSoundEffectsVolume = ::GetPrivateProfileIntA(
