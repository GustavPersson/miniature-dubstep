...
long WindowHandler::mOldMouseX = 0;
long WindowHandler::mOldMouseY = 0;
	...
	switch(msg)
	{
		...
		case WM_MOUSEMOVE:
			logic::safeThreadSafeQueueEvent(logic::EventDataPtr(myNew logic::EvtData_System_MouseMove(LOWORD(lParam), HIWORD(lParam),
				LOWORD(lParam) - WindowHandler::mOldMouseX, HIWORD(lParam) - WindowHandler::mOldMouseY)));
			WindowHandler::mOldMouseX = LOWORD(lParam);
			WindowHandler::mOldMouseY = HIWORD(lParam);
			return TRUE;
