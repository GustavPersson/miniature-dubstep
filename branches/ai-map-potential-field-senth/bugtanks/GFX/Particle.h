...
namespace gfx
{
	...
	public:
		 ...
		 * @param techIndex The tech index to be used
		 ...
		 */
		Particle(ID3D10Device *pDevice, Effect *pEffect, char* name, int techIndex, int passIndex, int initParticles, int maxChildParticles);
		...
		/**
		 * Sets position from where particles shall emit
		 * @param position Position from where particles emit
		 * @param actiave True if the emitter shall spawn new particles, else it will be idle
		 */
		void setEmitPosition(D3DXVECTOR3 position, bool activate);
	...
	protected:
		...
		ID3D10EffectScalarVariable			*mpFXParticleType;
