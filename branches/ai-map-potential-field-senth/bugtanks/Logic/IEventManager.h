...
/**
 * Interface classes for event managers, events and event listeners.
 */
...
public:
	...
	*/
	...
	/**
	* Returns the creator's network ID.
	*/
...
public:
	...
	**/
	explicit BaseEventData(const float timeStamp = GlobalTimer::getInstance()->getTime(utilities::Timer::ReturnType_MilliSeconds),
		int networkId = PlayerInfo::getInstance()->getNetworkId())
		: mTimeStamp(timeStamp), mNetworkId(networkId)
	{}

	/**
	* Constructor for serialized data. Takes a stream and network ID.
	* @param in The stream containing the event data
	* @param networkId The creator's network ID.
	* @param timeStamp The time of event creation.
	*/
	explicit BaseEventData(std::istrstream &in, int networkId = PlayerInfo::getInstance()->getNetworkId(),
	 ...
	 * Serializes the event for use in for example sending through networks.
	 */
	virtual void serialize(std::ostrstream &streamOut) const = 0;
