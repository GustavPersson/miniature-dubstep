...
public:
	...
	*/
	void setGoals(const PotentialSquare* newGoals, PathFinder::GoalTypes goalType);
	...
	*/
	...
	/**
	* Returns the unit offset depending on the position.
	* I.e. if the unit has the size 2x1 the offset will be -1, 0 from
	* the current position.
	*/
	utilities::MapCoordinate getGoalOffset(PathFinder::GoalTypes goalType) const;

	/**
	* Sets if we currently have our goal in the conflict map or not.
	* @param inConflictMap true if the specified goal type is in the conflict map
	* @param goalType the goal type we want to set
	*/
	inline void setGoalInConflictMap(bool inConflictMap, PathFinder::GoalTypes goalType)
	{
		mGoalInConflictMap[goalType] = inConflictMap;
	}

	/**
	* Returns if we currently have our specified goal in the conflict map or not
	* @param goalType the goal type we want to check
	* @return true if we currently have the specified goal in the conflict map or not.
	*/
	inline bool getGoalInConflictMap(PathFinder::GoalTypes goalType) const
	{
		return mGoalInConflictMap[goalType];
	}
...
private:
	...
	*/
	...
	bool mGoalInConflictMap[PathFinder::GoalType_Lim];
