...
public:
	...
	* @param unitSize the current unit size
	*/
	void removeFromConflictMap(const utilities::MapCoordinate& goalPosition, const logic::KeyType& keyId, GoalTypes goalType, const utilities::MapCoordinate& unitSize);
	...
	*/
	...
	/**
	* Friend class that needs the access to addToConflictMapFromEvent and removeFromConflictMapFromEvent
	* @see addToConflictMapEvent
	* @see removeFromConflictMapEvent
	*/
	friend class AiListener; 
...
private:
	...
	* @param unitSize the current unit size
	*/
	void removeFromConflictMapFromEvent(const utilities::MapCoordinate& goalPosition, const logic::KeyType& keyId, GoalTypes goalType, const utilities::MapCoordinate& unitSize);
	...
	* @param pAgentAi pointer to the agent we want to check a conflict with
	* @param goalType the goal type we want to check the conflict
	* @return the keyId of the other agent we have a conflict with
	*/
	logic::KeyType checkForConflicts(const AgentAi* pAgentAi, GoalTypes goalType) const;
